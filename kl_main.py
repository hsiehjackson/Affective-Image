...
def main():
    ...
    #loss_fn_kl = softCrossEntropy()
    loss_fn_kl = nn.KLDivLoss()
    metrics = nn.LogSoftmax()
    ...
    if MODE == 'train':
        ...
        for epoch in range(0, EPOCHS):
            ...
            for batch_idx, data in enumerate(train_loader):
                ...
                loss_ce = loss_fn_ce(outputs,target_label)
                #print(outputs.shape, labels.shape)
                loss_kl = loss_fn_kl(metrics(outputs),labels)
                loss = (1 - LAMDA) * loss_ce + LAMDA * loss_kl
